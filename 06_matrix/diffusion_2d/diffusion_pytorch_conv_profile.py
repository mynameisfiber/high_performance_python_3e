...
def run_experiment(num_iterations):
    ...
    conv = torch.nn.Conv2d(in_channels=1, out_channels=1, kernel_size=3, bias=False, padding_mode='circular', padding=1).to(device='cuda')
...
if __name__ == "__main__":
    with profile(activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA], profile_memory=True, record_shapes=True, with_flops=True) as prof:
        with record_function("pytorch+conv"):
