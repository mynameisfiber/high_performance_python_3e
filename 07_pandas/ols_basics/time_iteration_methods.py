...
print("Now using the numba fast path")
t1 = time.time()
#results = df.apply(ols_lstsq_raw, axis=1, raw=True)
results = df.apply(ols_lstsq_raw, axis=1, raw=True, engine="numba")
t2 = time.time()
print(f"Numba fastpath compilation {t2 - t1}")

t1 = time.time()
#results = df.apply(ols_lstsq_raw, axis=1, raw=True)
results = df.apply(ols_lstsq_raw, axis=1, raw=True, engine="numba")
t2 = time.time()
print(f"Numba fastpath {t2 - t1}")
