...
#SIZE_A, SIZE_B = 10_000, 320_000  # 24GB # 2nd ed
SIZE_A, SIZE_B = 10_000, 400_000  # 32GB # 3rd ed


# 202409 Ian was surprised to see 16 threads upon this program's start, it turns out that
# importing numpy with openblas spawns a number of threads for parallelism inside openblas
# and this can be controlled using e.g.
# OPENBLAS_NUM_THREADS=4 python np_shared.py (the threads in htop drop to 4 from 16)
# and openblas was diagnosed using:
#np.show_config()
# and it can be disabled for this diagnostic (in this shell) using
# https://stackoverflow.com/questions/51256738/multiple-instances-of-python-running-simultaneously-limited-to-35
# export OPENBLAS_NUM_THREADS=1
# python np_shared.py

# 202409 Ian was also surprised when trying to allocate a >50% RAM array and getting
# Bus Error (core dumped)
# https://stackoverflow.com/questions/77098807/allocating-large-shared-memory-leads-to-bus-error-core-dumped
# findmnt -o AVAIL,USED /dev/shm
# and learning that by default for shared memory, up to 50% RAM is allowed
# This can be overridden, but Ian didn't experiment
...
if __name__ == '__main__':
    ...
    input(f"Press a key to start workers using multiprocessing with {NBR_OF_PROCESSES=}...")
    print()
    ...
    input("Press return to start verifying")
