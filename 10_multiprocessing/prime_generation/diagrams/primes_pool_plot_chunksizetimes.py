...
if __name__ == "__main__":
    ...
    png_filename = "09_primes_pool_plot_chunksizetimes_1to50000_{}plottype{}.png".format(shuffle_in_filename, plot_type)

    if CREATE_DATA:
        time_per_chunksize = []
        if plot_type == 1:
            # plot the main effect:
            chunksizes = [1, 2, 3, 4, 5, 8, 10, 50, 100, 1000, 2500, 5000, 10000, 50000]
        if plot_type == 2:
        ...
        if mp_extra:
            mp_chunksize += 1
        pickle.dump((chunksizes, time_per_chunksize, mp_extra, mp_chunksize, mp_default_time), open(filename, 'wb'))
    else:
        chunksizes, time_per_chunksize, mp_extra, mp_chunksize, mp_default_time = pickle.load(open(filename, 'rb'))
        ...
        plt.tight_layout()
        print(f"Saving to {png_filename=}")
