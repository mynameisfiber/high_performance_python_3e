...
if __name__ == '__main__':
    ...
    print(f"keep_words set to {keep_words:,}")
    ...
    if online:
        ...
        wiki = WikiCorpus(inp, lower=False)  # takes about 9h on a macbook pro, for 3.5m articles (june 2011)
        # only keep the most frequent words (out of total ~8.2m unique tokens)
        #wiki.dictionary.filter_extremes(no_below=20, no_above=0.1, keep_n=DEFAULT_DICT_SIZE)
        wiki.dictionary.filter_extremes(no_below=0, no_above=1, keep_n=keep_words)
        # save dictionary and bag-of-words (term-document frequency matrix)
        #MmCorpus.serialize(outp + '_bow.mm', wiki, progress_cnt=10000, metadata=True)  # another ~9h # IAN commented out
        wiki.dictionary.save_as_text(outp + '_wordids.txt.bz2')
        # load back the id->word mapping directly from file
        # this seems to save more memory, compared to keeping the wiki.dictionary object from above
        1/0 # IAN let's crash out here, all we wanted were the word ids!
        #dictionary = Dictionary.load_from_text(outp + '_wordids.txt.bz2') # IAN commented out
    del wiki
