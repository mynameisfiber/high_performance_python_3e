...
if __name__ == "__main__":
    ...
    # load the words directly into the dict 
    t1 = time.time()
    words_dict = {k:n for (n,k) in enumerate(readers)}
    t2 = time.time()
    print("RAM after creating dict {:0.1f}MiB, took {:0.1f}s".format(memory_profiler.memory_usage()[0], t2 - t1))
    print("The dict contains {} words".format(len(words_dict)))

    word_to_check = text_example.WE_EXPECT_TO_SEE_2
    assert word_to_check in words_dict
    stmt=f"'{word_to_check}' in words_dict"
    exec("assert " + stmt)
    print(f"Checking that we'll see {word_to_check} in our words container")
    time_cost = sum(timeit.repeat(stmt="u'{word_to_check}' in words_dict",
                                  setup="from __main__ import words_dict",
                                  number=1,
